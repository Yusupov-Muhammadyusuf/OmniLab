...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (savedData) {
        try {
            ...
            const safetyPoints = reaction.safety.split('|').map((p: string) => p.trim()).filter((p: string) => p.length > 0);
            const safetyHTML = safetyPoints.map((p: string) => `<li class="mb-2"><i class="bi bi-shield-exclamation me-2"></i>${p}</li>`).join('');
