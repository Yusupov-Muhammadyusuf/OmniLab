...
export function updateAndDrawAmbientBubbles(cx, cy) {
    ...
    for (let i = bubbles.length - 1; i >= 0; i--) {
        ...
        let inside = false;
        if (state.currentVessel === 'beaker' && Math.abs(currentX - cx) < 85)
            inside = true;
        else if (state.currentVessel === 'tube' && Math.abs(currentX - cx) < 30)
            inside = true;
            ...
            if (Math.abs(currentX - cx) < (maxW / 2 - 10))
                inside = true;
        }
...
function drawPrecipitate(cx, liquidTopY, liquidBottomY, startX, endX, color) {
    if (!config.ctx)
        return;
    ...
    for (let index = 0; index < 42; index++) {
        ...
        config.ctx.fillStyle = config.hexToRgbA(color, 0.58 + (index % 3) * 0.1);
    ...
    const sedimentGradient = config.ctx.createLinearGradient(cx, liquidBottomY - sedimentHeight, cx, liquidBottomY);
    ...
    config.ctx.fillRect(startX, liquidBottomY - sedimentHeight, endX - startX, sedimentHeight);
    config.ctx.restore();
}
...
export function drawVesselAndFluid() {
    ...
    if (updatedState.burnerActive) {
        ...
        midGrad.addColorStop(1, 'rgba(255, 255, 150, 0)');
        midGrad.addColorStop(1, 'rgba(255, 255, 150, 0)');
...
export function renderChemicalVessel(app) {
